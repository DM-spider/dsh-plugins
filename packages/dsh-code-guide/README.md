...
「代码陪读」面板:打开一个脚本后,左侧面板内源码区与解读区双栏联动——逐函数给出通俗解读(作用 / 执行流程与数据流转 / 关键公式),点击函数卡片跳转对应代码行,点击代码行高亮对应解读;另提供该文件函数的 **mermaid 调用图**。解读由宿主侧默认模型路由(LLM)生成。

## 功能

- 标题栏圆形开关按钮(文件资源管理器开关旁边),点击打开左侧可拖宽面板(420px 起,左缘撑开对话区)
...
- 文件超过 40 KB 提示不支持;超过 3000 行只显示前 3000 行

## 依赖

- 宿主:`fs` 服务 + `llm` 服务(使用当前默认模型选择)+ `webServer`(与 GUI 同源的 `/plugins/code-guide/*` 路由)
