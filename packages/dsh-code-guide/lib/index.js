...
const SINGLE_PROMPT = [
  ...
  '{',
  '  "functions": [',
  '    {"name": "函数名(类方法写成 Class.method)", "start": 起始行号, "end": 结束行号, "summary": "一句话:这个函数做什么", "flow": "执行流程与数据流转,必须分步骤:每步用 1. 2. 3. 编号,每步一句话,不要混成一段", "formula": "关键公式或核心算法说明;没有则为空字符串"}',
  '  ],',
...
const EXPLAIN_PROMPT = [
  ...
  '{',
  '  "functions": [',
  '    {"name": "必须与清单完全一致的函数名", "summary": "一句话:这个函数做什么", "flow": "执行流程与数据流转,必须分步骤:每步用 1. 2. 3. 编号,每步一句话,不要混成一段", "formula": "关键公式或核心算法说明;没有则为空字符串"}',
  '  ],',
  '  "callEdges": [["调用方函数名", "被调用函数名"]]',
  '}',
  '要求:',
