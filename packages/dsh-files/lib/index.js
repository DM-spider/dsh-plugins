...
const EXPLAIN_PROMPT = [
  ...
  '{',
  '  "functions": [',
    '    {"name": "必须与清单完全一致的函数名", "summary": "一句话:这个函数做什么", "flow": [{"start": 本步骤对应的起始行号, "end": 本步骤对应的结束行号, "text": "先用通俗语言描述目的,关键标识符用反引号紧跟在描述后面做注释"}], "formula": "关键公式或核心算法说明,多条公式用分号分隔,每条格式为 代码公式 → 中文公式(用中文变量名重写同一公式);没有则为空字符串。示例: profit = sell_price - buy_price → 利润 = 卖出价 - 买入价; sharpe = mean(returns) / std(returns) → 夏普比率 = 收益均值 / 收益标准差"}',
  '  ],',
  '  "callEdges": [["调用方函数名", "被调用函数名"]]',
  '}',
  '要求:',
