...
const SINGLE_PROMPT = [
  ...
  '{',
  '  "functions": [',
  '    {"name": "函数名(类方法写成 Class.method)", "start": 起始行号, "end": 结束行号, "signature": "函数定义行的原始内容(去掉行首缩进,逐字照抄,绝不能改写;装饰器则从第一行装饰器开始)", "summary": "一句话:这个函数做什么", "flow": [{"start": 本步骤对应的起始行号, "end": 本步骤对应的结束行号, "text": "先用通俗语言描述目的,关键标识符用反引号紧跟在描述后面做注释"}], "formula": "关键公式或核心算法说明;没有则为空字符串"}',
  '  ],',
...
const EXPLAIN_PROMPT = [
  ...
  '{',
  '  "functions": [',
  '    {"name": "必须与清单完全一致的函数名", "summary": "一句话:这个函数做什么", "flow": [{"start": 本步骤对应的起始行号, "end": 本步骤对应的结束行号, "text": "先用通俗语言描述目的,关键标识符用反引号紧跟在描述后面做注释"}], "formula": "关键公式或核心算法说明;没有则为空字符串"}',
  '  ],',
  '  "callEdges": [["调用方函数名", "被调用函数名"]]',
  '}',
  '要求:',
